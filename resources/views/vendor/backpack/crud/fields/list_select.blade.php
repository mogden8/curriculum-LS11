...
@include('crud::fields.inc.wrapper_start')
    ...
    <input type="text"  list="{{ $field['label'] }}"
        name="{{ $field['name'] }}"
            ...
            @if (count($options))
                @foreach ($options as $connected_entity_entry)
                    @if($current_value == $connected_entity_entry->getKey())
                        <option value="{{ $connected_entity_entry->{$field['foreign-ref']} }}" selected>{{ $connected_entity_entry->{$field['attribute']} }}</option>
                    @else
                        <option value="{{ $connected_entity_entry->{$field['foreign-ref']} }}">{{ $connected_entity_entry->{$field['attribute']} }}</option>
                    @endif
                @endforeach
            @endif
        </datalist>
