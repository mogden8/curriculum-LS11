        ...
        @if (count($options))
            @foreach ($options as $connected_entity_entry)
                @if($current_value == $connected_entity_entry->getKey())
                    <option value="{{ $connected_entity_entry->getKey() }}" selected>{{ strip_tags($connected_entity_entry->{$field['attribute']}) }}</option>
                @else
                    <option value="{{ $connected_entity_entry->getKey() }}">{{ strip_tags($connected_entity_entry->{$field['attribute']}) }}</option>
                @endif
            @endforeach
        @endif
    </select>
